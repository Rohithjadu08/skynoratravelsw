...
function transformProperties(props: Record<string, any>): Record<string, any> {
  ...
  for (const [key, val] of Object.entries(props)) {
    const propCopy: Record<string, any> = {
      type: mapType(val.type),
      description: val.description || "",
    };
    if (val.enum) propCopy.enum = val.enum;
    if (val.items) propCopy.items = { type: mapType(val.items.type) };
    result[key] = propCopy;
  }
  return result;
}
...
export function getGeminiTools(): FunctionDeclaration[] {
  return (TRAVEL_TOOLS as any[]).map((t) => ({
    name: t.name,
