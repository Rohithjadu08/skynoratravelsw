...
class ItineraryAgent {
    build(input) {
        ...
        const zero = { amount: 0, currency: input.currency };
        const date = new Date(`${input.startDate}T00:00:00Z`);
        const label = { dataType: "ai_recommended", source: "SkyNora itinerary composer", retrievedAt: new Date().toISOString() };
        let total = 0;
        const days = Array.from({ length: input.days }, (_, index) => {
            ...
            return {
                ...
                meals: restaurant ? [{ id: restaurant.id, label: restaurant.label, time: "13:00", restaurantName: restaurant.name, cuisine: restaurant.cuisine, estimatedCost: zero, mealType: "lunch", address: restaurant.address }] : [],
...
exports.ItineraryAgent = ItineraryAgent;
//# sourceMappingURL=itinerary.agent.js.map